...
export const FormCreatePostBtn = ({user, setModalOpen}: {user: any, setModalOpen: (value: boolean) => void}) => {
  ...
  return <form onSubmit={handleSubmit} className="flex gap-3 flex-col items-start  p-4">
    <img
    onClick={() => router.push(`/profile/${user.id}`)}
    src={user && user.image} alt="user pdp" className="w-10 h-10 rounded-full cursor-pointer" />

    <div className="flex flex-1 w-full flex-col gap-2 py-1">
        <textarea 
        ref={textareaRef}
        onChange={handleChange}
        value={content}
        placeholder="Quoi de neuf ?!"
        className={`bg-transparent w-full text-lg outline-none font-normal ${
    ...
    <div className='flex items-center border-t w-full border-white/10 justify-between py-2'>
